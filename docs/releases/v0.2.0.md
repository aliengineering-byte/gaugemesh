...
`resilireplay@0.7.0` package. The required clean control and ResiliReplay's
trace-level synthetic timeout and deterministic-error retry cases pass with
cleanup and zero duplicate effects; those mutations are not injected into the
MCP wire. The full thirteen-scenario aggregate remains honestly `PARTIAL`, and
no MCP-RES claim is made.
