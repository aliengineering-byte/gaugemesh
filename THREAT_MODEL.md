...
| A local PROCESS worker escapes its output directory or emits plausible invalid output | Qualification uses a temporary permitted root, rejects absolute/traversal and symlink paths, creates new files, bounds process time and bytes, and verifies result and manifest independently | This is test evidence for a harmless worker, not a general OS sandbox, hostile-code boundary, child-tree containment guarantee, or shipped production executor |
